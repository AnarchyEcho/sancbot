...
{
    [Group]
    public partial class StarboardCommands(DiscordSocketClient client)
        : NadekoModule<StarboardService>
    {
        ...
        public async Task Starboard()
        {
            ...
            NadekoInteractionBase CreateButton(
                ButtonBuilder btn,
                Func<SocketMessageComponent, Task> onClick
            ) =>
                new NadekoButtonInteractionHandler(
                    client,
                    ctx.User.Id,
                    btn,
                    onClick,
            ...
            NadekoInteractionBase CreateSelect(
                SelectMenuBuilder menu,
                Func<SocketMessageComponent, Task> onSelect
            ) =>
                new NadekoButtonSelectInteractionHandler(
                    client,
                    ctx.User.Id,
                    menu,
                    onSelect,
            ...
            ComponentBuilder BuildPanelComponents()
            {
                ...
                selfStarBtn.WithStyle(
                    settings?.AllowSelfStar == true ? ButtonStyle.Success : ButtonStyle.Secondary
                );
                botsBtn.WithStyle(
                    settings?.AllowBots == true ? ButtonStyle.Success : ButtonStyle.Secondary
                );
                statusBtn.WithStyle(
                    settings is null || settings.IsEnabled
                        ? ButtonStyle.Success
                        : ButtonStyle.Danger
                );
            ...
            {
                if (
                    await AwaitChatInputAsync(
                        smc,
                        strs.starboard_prompt_threshold,
                        static s =>
                            int.TryParse(s, out var n)
                            && n
                                is >= StarboardConsts.MIN_THRESHOLD
                                    and <= StarboardConsts.MAX_THRESHOLD
                    )
                    is not { } input
                )
                    return;
            ...
            async Task OnEmoteAsync(SocketMessageComponent smc)
            {
                if (
                    await AwaitChatInputAsync(
                        ...
                        static s =>
                            StarboardService.ParseEmote(s.AsSpan().Trim().ToString()) is not null
                    )
                    is not { } input
                )
                    return;
        ...
        {
            var eb = CreateEmbed().WithOkColor().WithTitle(GetText(strs.starboard_title));
            ...
            eb.AddField(GetText(strs.starboard_channel), $"<#{settings.ChannelId}>", true)
                ...
                .AddField(
                    GetText(strs.starboard_self_star),
                    settings.AllowSelfStar ? "✅" : "❌",
                    true
                )
        ...
        )
        {
            ...
            var cancelInter = new NadekoButtonInteractionHandler(
                client,
                ctx.User.Id,
                cancelBtn,
                _ =>
            ...
            var promptMsg = await smc.FollowupAsync(
                GetText(prompt),
            ...
            try
            {
                return await GetUserInputAsync(ctx.User.Id, ctx.Channel.Id, validate);
            }
            finally
            {
